#!/usr/bin/env python3
"""
Bell State benchmark on IQM Emerald QPU.

Creates a 2-qubit Bell state |Φ+⟩ = (|00⟩ + |11⟩) / √2
Runs on real IQM superconducting quantum hardware.

IQM Emerald: 53-qubit superconducting QPU (larger than Garnet's 20 qubits)

COST: IQM is very affordable (similar to Rigetti)
- Per-task: $0.30
- Per-shot: $0.00145 (same as Garnet)
...
# IQM Emerald ARN (eu-north-1) - 53 qubit superconducting
...
S3_PREFIX = "benchmarks/iqm-emerald"
...
def run_bell_state_emerald(shots: int = SHOTS) -> tuple[dict, float | None]:
    """
    Run Bell state circuit on IQM Emerald.

    Returns:
        Tuple of (result_data, quantum_task_duration)
    """
    aws_session = get_aws_session()
    device = AwsDevice(IQM_EMERALD_ARN, aws_session=aws_session)
...
def main():
    print("Bell State Benchmark - IQM Emerald QPU")
    print("=" * 55)
    print(f"Device: IQM Emerald (superconducting, 53 qubits)")
    ...
    # Run benchmarks
    print("Running benchmarks on IQM Emerald...")
    ...
    for i in range(num_runs):
        ...
        try:
            result_data, quantum_time = run_bell_state_emerald(shots=SHOTS)
    ...
    # Save results
    output_dir = Path(__file__).parent.parent / "results" / "m0-benchmarking" / "iqm-emerald" / "bell-state"
    ...
    print("\n" + "=" * 55)
    print("Comparison to Other IQM Backends:")
    print("=" * 55)
    print(f"IQM Garnet (20 qubits):   7.7s, 97.4% fidelity, ~$1.75")

    if successful:
        wall_times = [r.wall_time_seconds for r in successful]
        emerald_mean = sum(wall_times) / len(wall_times)
        last_counts = successful[-1].result_data.get("counts", {})
        if last_counts:
            total = sum(last_counts.values())
            bell_states = last_counts.get("00", 0) + last_counts.get("11", 0)
            emerald_fidelity = bell_states / total
            print(f"IQM Emerald (53 qubits):  {emerald_mean:.1f}s, {emerald_fidelity:.1%} fidelity, ~$1.75")
